...
DATA4 = [{'text': 'MISSOURI', 'coordinate': [[36, 34], [84, 34], [84, 60], [36, 60]]}, {'text': 'ILLINOIS', 'coordinate': [[36, 34], [84, 34], [84, 60], [36, 60]]}]
...
class TestParseTextMethods(unittest.TestCase):
    ...
    def test_connectTest(self):
      self.assertEqual(self.parser._connectText(DATA),'startright to startbelow startMISSOURI')
    ...
    def test_findStateName(self):
      ...
      self.assertEqual(self.parser.state, {'state':'MISSOURI', 'coordinate':[[1000, 1000], [1000, 1000], [10000, 10000], [10000, 10000]]})
    ...
    def test_getPosFromCoordinates(self):
      self.assertEqual(self.parser._getPosFromCoordinates(DATA[0]['coordinate']), {'leftX': 36, 'rightX': 84, 'topY': 60, 'bottomY': 34})

    def test_findOffset(self):
      self.assertEqual(self.parser._findOffset(DATA[0]['coordinate'],DATA[1]['coordinate']), {'xLeftOffSet': 1.0208333333333333, 'yMiddleOffSet': 0.23076923076923078})

    def test_findAllOffset(self):
      self.parser2.state = {'state':'MISSOURI','coordinate':[[1000, 1000], [1000, 1000], [10000, 10000], [10000, 10000]]}
      self.parser2._findAllOffsets()
      self.assertEqual(self.parser2.offsets, {'first_name': {'xLeftOffSet': 0.009523809523809525, 'yMiddleOffSet': 2.532608695652174}, 'last_name': {'xLeftOffSet': 0.05, 'yMiddleOffSet': 2.1739130434782608}, 'issue_date': {'xLeftOffSet': 0.8523809523809524, 'yMiddleOffSet': 4.826086956521739}, 'expiration_date': {'xLeftOffSet': 0.9738095238095238, 'yMiddleOffSet': 1.3804347826086956}, 'address_one': {'xLeftOffSet': 0.011904761904761904, 'yMiddleOffSet': 2.869565217391304}, 'address_two': {'xLeftOffSet': 0.047619047619047616, 'yMiddleOffSet': 3.141304347826087}})

    def test_findPosFromOffset(self):
      self.assertEqual(self.parser._findPosFromOffset(DATA[0]['coordinate'], {'xLeftOffSet': 1.0208333333333333, 'yMiddleOffSet': 0.23076923076923078}), {'targetLeftX': 85.0, 'targetMiddleY': 53.0})

    def test_getFieldFromPos(self):
      self.parser.devX = 140
      self.parser.devY = 10
      self.assertEqual(self.parser._getFieldFromPos({'targetLeftX': 85.0, 'targetMiddleY': 53.0}), {'coordinate': [[36, 34], [84, 34], [84, 60], [36, 60]], 'text': 'start'})
      self.assertEqual(self.parser._getFieldFromPos({'targetLeftX':88888.0, 'targetMiddleY':8888.0}), None)

    def test_findAllFields(self):
      self.parser3._getStateData()
      self.parser3._findAllOffsets()
      self.parser3._findAllFields()
      self.assertEqual(self.parser3.result, {'first_name': '2 SAMANTHA DRIVER', 'last_name': '1SAMPLE', 'issue_date': '4a ISS 01/05/2020', 'expiration_date': '4b EXP02/14/2026', 'address_one': '8 123 SAMPLE ST', 'address_two': 'JEFFERSON CITY, MO 65105'})
    ...
    def test_cleanRawData(self):
      ...
      self.assertEqual(self.parser3.result, {'first_name': 'SAMANTHA DRIVER', 'last_name': 'SAMPLE', 'issue_date': '01/05/2020', 'expiration_date': '02/14/2026', 'address_one': '123 SAMPLE ST', 'address_two': 'JEFFERSON CITY, MO 65105'})
